...
/**
 * Reponse de `GET /api/previsions/produits-alimentaires-eligibles`.
 *
 * Pas de champ booleen dedie "site sans aucun produit ALIMENT actif" : cet
 * endpoint n'a ni pagination ni filtre variable (toujours "tous les Produit
 * ALIMENT actifs du site"), donc `data: []` est deja un signal non ambigu
 * (ERR-173/ERR-185 : un `0`/vide n'est un mensonge que s'il PEUT aussi
 * signifier "pas encore charge" ou "filtre" — ce n'est pas le cas ici, la
 * requete HTTP elle-meme a reussi ou a leve une erreur reseau distincte).
 * C'est a l'UI de rendre ce cas visible explicitement (etat vide dedie,
 * jamais une liste de cases a cocher silencieusement absente) — cf. ADR-053
 * §18.
 */
export interface ProduitsAlimentairesEligiblesResponse {
  data: ProduitAlimentaireEligibleDTO[];
}
...
// ---------------------------------------------------------------------------
// Prévisions — Aliments (calibre, champs article fusionnés sur le calibre)
// ---------------------------------------------------------------------------

/**
 * DTO pour créer un calibre d'aliment prévisionnel — les champs article
 * (`libelle`, `poidsSacKg`, `prixSacFCFA`, `produitId`) sont fusionnés
 * directement sur ce DTO depuis la suppression du modèle intermédiaire
 * `AlimentArticlePrevision` (chaque calibre = un seul article, cas qui
 * était déjà le seul cas réellement utilisé).
...
export interface CreateAlimentPrevisionDTO {
   ...
   * en entrée comme en base : aucune source de repli automatique n'existe,
   * l'utilisateur peut le laisser non configuré et le renseigner plus tard,
   * mais tout calcul qui en dépend rejette tant qu'il est absent.
   */
...
/**
 * DTO pour modifier les champs d'un calibre existant (coefficient de besoin
 * et/ou champs article). `tailleGranule` n'est volontairement pas
 * modifiable ici : c'est l'identité du calibre
 * (`@@unique([scenarioId, tailleGranule])`) — la changer reviendrait à
 * recréer un autre calibre, pas à éditer celui-ci.
 */
export interface UpdateAlimentPrevisionDTO {
  ...
  poidsSacKg?: number;
  prixSacFCFA?: number;
  /** `null` explicite pour délier le calibre du catalogue produit */
  produitId?: string | null;
}

/** Réponse d'un calibre d'aliment prévisionnel. */
export type AlimentPrevisionResponse = AlimentPrevision;
