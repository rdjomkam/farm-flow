/**
 * GET /api/reproduction/kpis — KPIs complets de la chaine de reproduction
 *
 * Retourne les indicateurs cles de performance pour le module reproduction :
 * totaux, taux, geniteurs, lots et production mensuelle.
 *
 * Query params :
...
import { getReproductionKpis } from "@/lib/queries/reproduction-stats";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const kpis = await getReproductionKpis(
      auth.activeSiteId,
      dateDebut,
      dateFin
    );
    ...
    return handleApiError(
      "GET /api/reproduction/kpis",
      error,
      "Erreur serveur lors du calcul des KPIs de reproduction."
    );
  }
}
