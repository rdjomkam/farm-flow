...
/**
 * BackofficeSiteModulesEditor — editeur des modules d'un site (backoffice).
 *
 * Identique a AdminSiteModulesEditor mais utilise /api/backoffice/sites/[id]/modules.
...
interface BackofficeSiteModulesEditorProps {
  siteId: string;
  enabledModules: SiteModule[];
  onSaved?: (modules: SiteModule[]) => void;
}

export function BackofficeSiteModulesEditor({
  siteId,
  enabledModules: initialModules,
  onSaved,
}: BackofficeSiteModulesEditorProps) {
  ...
  async function handleSave() {
    setLoading(true);
    try {
      const res = await fetch(`/api/backoffice/sites/${siteId}/modules`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error ?? data.message ?? t("modulesEditor.updateError"));
      }
      const data = await res.json();
      toast({ title: t("siteStatus.toastSuccess"), variant: "success" });
      onSaved?.(data.enabledModules ?? [...enabled]);
    } catch (err) {
      toast({
        title: t("siteStatus.toastError"),
        description: err instanceof Error ? err.message : t("siteStatus.toastErrorDefault"),
        variant: "error",
      });
    } finally {
      setLoading(false);
