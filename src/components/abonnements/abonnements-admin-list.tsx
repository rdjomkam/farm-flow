          ...
          <tbody>
            {abonnements.map((a, i) => (
              <tr key={a.id} className={i % 2 === 0 ? "bg-card" : "bg-muted/10"}>
                <td className="px-4 py-3 font-medium text-foreground">{a.site?.name ?? t("admin.noSite")}</td>
                      ...
                      <Dialog open={activationId === a.id} onOpenChange={(open) => !open && setActivationId(null)}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              {t("admin.forceActivationDesc", { siteName: a.site?.name ?? t("admin.noSite") })}
                            </DialogDescription>
                          </DialogHeader>
                          <DialogFooter>
                      ...
                      >
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              {t("admin.cancelSubscriptionDesc", { siteName: a.site?.name ?? t("admin.noSite") })}
                            </DialogDescription>
                          </DialogHeader>
                          {annulationError && (
        ...
        {abonnements.map((a) => (
          <div key={a.id} className="bg-card border border-border rounded-xl p-4 space-y-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-foreground">{a.site?.name ?? t("admin.noSite")}</p>
