...
**Script de constat à écrire pour la prod** (`scripts/audits/px-audit-signatures-corrompues.*`), suivant le pattern de `scripts/data-fixes/gd3-apply.sh` (bash + SQL séparés, mode lecture seule ici donc pas de backup nécessaire) :
...
| **PX.5** | SCHEMA/QUERIES (lecture seule) | `@db-specialist` | Script `scripts/audits/px-audit-signatures-corrompues.*` (Node + zlib, réutilise `isDecodableImage()`) : constat read-only en base de prod sur les 4 colonnes concernées. Aucune mutation. Rapport à `docs/bugs/` ou `docs/analysis/`. |
