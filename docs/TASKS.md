...
| **48** | UI cleanup + messages + flow création site + UI bloqué | 48.1–48.5 | `FAIT` |
...
Voir : [SPRINT-PR2-ter-PREVISIONS.md](sprints/SPRINT-PR2-ter-PREVISIONS.md)

**Statut :** `FAIT` (**2 stories**)
**Objectif :** lever deux obstacles constatés à l'écran pendant le rejeu intégral du plan de référence Excel par l'interface, tous deux dans `src/components/previsions/`.
**Contrainte transverse :** interdiction de toucher au moteur `src/lib/previsions/` — la recette doit rester à **1270 tests / 0 écart**.
**Hors périmètre :** rapprochement prévu/réel, vues de comparaison, exports, reprévision.

| Story | Type | Sujet | Pipeline | Statut |
|-------|------|-------|----------|--------|
| PR2ter.1 | UI + API | Reporter une charge sur plusieurs mois (21 navigations / 84 frappes pour 4 nombres) | @pre-analyst → @developer → @tester → @code-reviewer | `FAIT` |
| PR2ter.2 | BUGFIX | Deux bugs de cycle de vie des dialogues : perte de saisie au clic extérieur (Haute) + state non réinitialisé à la réouverture, saisie concaténée (Critique) | @pre-analyst → @developer → @tester → @code-reviewer → @knowledge-keeper | `FAIT` |

**PR2ter.1 — pré-analyse faite, verdict `GO AVEC RÉSERVES`** (`docs/analysis/pre-analysis-story-PR2ter.1.md`). Le choix laissé ouvert par le 3ᵉ critère d'acceptation est tranché en faveur d'une **route de report en lot** (`POST /api/previsions/postes/[id]/charges/reporter`) plutôt que d'une boucle côté client. La story **s'étend donc en UI + API** (nouvelle route et son schéma de validation, tests d'API en plus des tests d'UI).

**PR2ter.2 — pré-analyse faite, verdict `GO`** (`docs/analysis/pre-analysis-story-PR2ter.2.md`). Cause racine du Bug B confirmée : `setForm(EMPTY_STATE)` n'est appelé que dans la **branche de succès** de la soumission — jamais sur « Annuler », jamais sur `onOpenChange` ; le state survit donc à toute fermeture qui n'est pas une validation réussie. `generer-plan-dialog.tsx` est le **seul** dialogue du module appliquant déjà le bon patron : il sert de référence pour corriger les autres.

### Clôture du Sprint PR2-ter

**Pipelines exécutés et rapports produits :**
- **PR2ter.1** (UI + API) : @pre-analyst → @developer → @tester → @code-reviewer — `docs/analysis/pre-analysis-story-PR2ter.1.md`, `docs/tests/rapport-story-PR2ter.1.md`, `docs/reviews/review-story-PR2ter.1.md`
- **PR2ter.2** (BUGFIX) : @pre-analyst → @developer → @tester → @code-reviewer → @knowledge-keeper — `docs/analysis/pre-analysis-story-PR2ter.2.md`, `docs/tests/rapport-story-PR2ter.2.md`, `docs/reviews/review-story-PR2ter.2.md`

**Verdicts :**
- **PR2ter.2 — `VALIDÉ`**, aucune réserve bloquante. Deux réserves non bloquantes : commentaire à clarifier sur le `setState` pendant rendu de `scission-dialog.tsx` (**Basse**, documentaire) ; blocage **silencieux** du garde de fermeture, sans feedback visuel (**Moyenne**, UX, à traiter en sprint de polish).
- **PR2ter.1 — `VALIDÉ AVEC RÉSERVES`**, aucune bloquante. Deux réserves **Basses** : aperçu calculé sur un **instantané client** sans verrou optimiste (risque identique à celui de la route unitaire préexistante, **pas une régression**) ; bouton « mois suivant » de `charges-tab.tsx` sans **borne haute** sur `horizonMois` — bug **pré-existant de PR2.3**, que le nouveau dialogue rend visible via le message trompeur « Horizon du plan indisponible » (à traiter en polish).
...
- **Leçon commune** : ces bugs vivent dans le **cycle de vie du composant**, pas dans la logique métier — aucun des 7487 tests ne les attrapait. **Règle retenue** : tout dialogue de formulaire doit avoir un test « ouvrir → saisir → fermer → rouvrir → champs vides », accompagné du test « clic extérieur **SANS** saisie → le dialogue se ferme bien ».

**Vérification de fin de sprint (sorties réelles rejouées) :**
- `npx vitest run` → **277 fichiers** (273 passés + 4 skippés), **7633 tests** (7588 passés, 19 skippés, 26 todo), **0 échec** — contre 267 fichiers / 7487 tests avant sprint (**7487 → 7633 tests, 0 échec**, +10 fichiers).
- `npx vitest run src/lib/previsions/__tests__/recette` → **1270 tests / 0 écart** (inchangé — contrainte moteur tenue).
- `npm run build` → **exit 0** : compilation + TypeScript + 169/169 pages OK.

**Périmètre : tenu.** Une alerte initiale sur 9 fichiers modifiés sous `src/lib/previsions/` a été investiguée et déclarée **faux positif** : ces diffs proviennent des stories **PR2.1** et **PR2bis.3** restées non committées au démarrage de PR2-ter (chaque hunk cite littéralement PR2.1 ou PR2bis.3), et non de PR2-ter, qui opère exclusivement dans `src/components/previsions/`, `src/app/api/previsions/` et `src/hooks/`. Aucun rapprochement prévu/réel, aucune vue de comparaison, aucun export ni reprévision n'est apparu.

**Réserves reportées (PAS des stories de ce sprint).** Aucune réserve bloquante. À reporter au prochain sprint de polish : feedback visuel du garde de fermeture (**Moyenne**, UX) et borne haute du navigateur de mois de `charges-tab.tsx` (**Basse**, pré-existant).

---

## Sprint PR2-quinquies — Compléter la vue Prévisions mensuelle
Voir : [SPRINT-PR2-quinquies-PREVISIONS.md](sprints/SPRINT-PR2-quinquies-PREVISIONS.md)

**Statut :** `FAIT` (**5 stories** — toutes `FAIT` — **+ 1 correctif post-livraison hors périmètre initial**) — **review de sprint `VALIDÉ AVEC RÉSERVES`, aucune réserve Critique ni Haute**
**Objectif :** la feuille « Prévisions » du classeur de référence `prisma/fixtures/previsions/Previsions_Elevage_Silure_v12.xlsx` porte **30 lignes d'indicateurs** ; l'application n'en affiche que **8** (Revenus, Coût aliments, Coût alevins, Charges réparties, Investissements, Dépenses totales, Apports, Solde cumulé). **Cible corrigée (comptage direct des `LigneDescriptor`) : le passage de 8 à 19 lignes rendues** — Résultat (4) + Production (3) + Aliments (2 statiques + **3 dynamiques selon le nombre de calibres**) + Entrées & dépenses détaillées (7) — couvrant **17 des 30 lignes du classeur**, les **13 restantes (lignes 11-23, détail par vague) étant explicitement exclues**. Le chiffre **30** est celui du classeur source, **jamais** un nombre de lignes affichées. Ce sprint complète la vue mensuelle, corrige le total de la ligne cumulative, solde la dette `resultats.epargne` dans le moteur et ajoute les ventilations demandées par l'utilisateur.
**Levier décisif :** la plupart des séries manquantes sont **déjà dans le jeu d'or** (`entrees.empoissonneT`, `entrees.ventesT`, `besoinsAliments.totalKg`, `besoinsAliments.sacsTotal`, `besoinsAliments.sacsParGranulometrie`, `depenses.alevinsCommandes`, `resultats.totalEntrees`, `resultats.resultat`, `resultats.epargne`) — donc vérifiables **au franc près**. Toute série exposée qui existe dans les fixtures **DOIT** être comparée au jeu d'or (tolérance **0** sur les entiers, **≤ 1 FCFA** sur les montants). Le détail par vague (lignes 11-23) n'est **pas** extrait : s'il est exposé, il faut étendre `prisma/fixtures/previsions/extract-golden.py`.
**Contrainte transverse :** le moteur `src/lib/previsions/` n'est modifiable que par **PR2q.2**. La recette doit finir à **≥ 1270 tests / 0 écart**. Le scénario `EXCEL-V12` en base (3 calibres G1/G2/G3, 19 vagues, 602 500 alevins, 241 t) est le jeu de test de l'utilisateur : **aucune de ses données ne doit être modifiée**.
**Hors périmètre :** rapprochement prévu/réel, vues de comparaison, reprévision glissante, exports.

| Story | Type | Sujet | Pipeline | Statut |
|-------|------|-------|----------|--------|
| PR2q.1 | BUGFIX | Corriger la colonne Total de la ligne cumulative (`Prévisions!W35 = V35`, pas une somme) | @developer → @code-reviewer | `FAIT` |
| PR2q.2 | QUERIES/moteur | L'épargne dans le moteur — `epargne(M) = max(0, resultat(M)) × tauxEpargnePct` (§5.6, 30 % au classeur) | @pre-analyst → @db-specialist → @developer → @tester → @code-reviewer → @knowledge-keeper | `FAIT` |
| PR2q.3 | UI | Compléter la vue mensuelle : **8 → 19 lignes rendues** (17 des 30 lignes du classeur) dans l'ordre du classeur, regroupement lisible obligatoire (§7.1) | @pre-analyst → @developer → @tester → @code-reviewer | `FAIT` |
| PR2q.4 | UI | Ventilations demandées par l'utilisateur : apports par `TypeApportCapital`, dépenses par `PostePrevision` — ajout au classeur, total ventilé = ligne agrégée (à tester) | @pre-analyst → @developer → @tester → @code-reviewer | `FAIT` |
| PR2q.5 | REVIEW | Review de sprint → `docs/reviews/review-sprint-PR2-quinquies.md` | @code-reviewer → @knowledge-keeper | `FAIT` |
| — | CORRECTIF | Correctif post-livraison : titres de section invisibles au défilement horizontal (détecté en **navigateur réel**, invisible en jsdom) | @developer | `FAIT` |

**Contraintes transverses :** i18n `fr` et `en` complètes, accents corrects, aucune chaîne en dur, clés mortes supprimées dans les deux langues ; formats §7.4 (séparateur de milliers, aucune décimale sur les montants, zéros en « – », négatifs en rouge, tonnages à une décimale, unité dans le libellé de ligne) ; boutons d'explication §7.4 « d'où vient ce nombre » sur **chaque** nouvelle ligne calculée ; ne pas défaire la transposition indicateurs-en-lignes / mois-en-colonnes, la colonne collante ni le défilement horizontal livrés juste avant ; vue carte mobile cohérente avec les nouvelles lignes.

**Vérification de fin :** `npx prisma migrate deploy` ; `npx vitest run` (base avant sprint : **282 fichiers, 7666 tests hors DB-gated, 0 échec**) ; recette `src/lib/previsions/__tests__/recette` **≥ 1270 / 0 écart** ; `npm run build`.

**Gouvernance :** seul @status-updater écrit dans `docs/sprints/` et `docs/TASKS.md`.

**⚠️ Collision d'identifiants de stories.** Le sprint **PR2-quater** utilise **déjà** `PR2q.1` à `PR2q.6` pour des sujets **sans rapport**, **tous `FAIT`**. Toute référence à un identifiant `PR2q.*` doit préciser le sprint (quater ou quinquies). Renommage éventuel à trancher par le @project-manager.

**PR2q.1 — `FAIT`.** Implémentée par @developer, review @code-reviewer **VALIDÉ**, une seule réserve de sévérité **Basse**, non bloquante (la robustesse « dernière valeur non vide » n'est pas couverte par un test dédié — sans impact actuel, `MoisProjectionDTO.soldeFCFA` étant non-optionnel). Mécanisme retenu : champ `totalMode: "somme" | "derniereValeur"` porté par la structure `LIGNE_KEYS` de `src/components/previsions/previsions-mensuelles-tab.tsx`, consommé uniformément par `calculerTotalLigne()` **sans branche conditionnelle par nom de ligne** — une ligne cumulative ajoutée par PR2q.3/PR2q.4 n'a qu'à déclarer ce champ. Seule `soldeFCFA` porte `"derniereValeur"`. **2 tests de non-régression** ajoutés dans `src/components/previsions/__tests__/previsions-mensuelles-tab.test.tsx`. `npx vitest run` → 277 fichiers passés / 5 skipped, **7668 tests passés** / 21 skipped / 26 todo, **0 échec** ; `npm run build` → **exit 0**.

**PR2q.2 — `FAIT`.** Pipeline amendé **exécuté intégralement** (@pre-analyst → @db-specialist → @developer → @tester → @code-reviewer → @knowledge-keeper). Fonction pure ajoutée dans `src/lib/previsions/tresorerie.ts` : `calculerEpargne(resultat, tauxEpargnePct) = Decimal.max(0, resultat).times(tauxEpargnePct).dividedBy(100)` — aucun I/O, `Decimal` strict. `resultatFCFA` et `epargneFCFA` ajoutés à `MoisProjectionResult` dans `src/lib/previsions/route-orchestration.ts` ; l'identité `resultatFCFA = revenusFCFA + apportsFCFA − depensesFCFA` = delta de `calculerTresorerieMensuelle` est **prouvée numériquement**, pas supposée. **Recette : 1270 → 1482 tests, 0 écart** (+42 pour `resultats.epargne` sur les 21+21 mois des 2 fixtures ; le reste pour une nouvelle « Section C » appliquant ERR-142 — comparaison des séries mensuelles `mois[]` de la vraie fonction publique `calculerProjectionScenario`, zone jusqu'ici non recettée où 3 bugs Haute s'étaient déjà logés). **@tester** : suite rejouée 2 fois (`277 passed | 5 skipped`, `7880 passed`, 0 échec), détection de régression prouvée par mutation volontaire de `calculerEpargne` (11 tests tombent, restauration vérifiée), et **gap ERR-141 comblé** qu'aucun test existant ne couvrait — nouveau test `src/lib/queries/__tests__/previsions-scenario-loader-tauxepargne-e2e.test.ts` faisant transiter deux valeurs différentes du taux par le chemin complet DB → loader → `calculerProjectionScenario` ; rapport `docs/tests/rapport-story-PR2q2.md`. **@code-reviewer : VALIDÉ**, aucune réserve Critique ni Haute — une seule réserve **Basse et informative, hors périmètre** : `updateParametresPrevision` reste un `findFirst({ siteId })` suivi d'un `update({ where: { scenarioId } })`, donc pas strictement atomique au sens R4 (pattern **préexistant**, non introduit par cette story) ; rapport `docs/reviews/review-story-PR2q2.md`. Instabilité de `src/components/previsions/__tests__/scenario-form-dialog.test.tsx` observée une fois sous charge parallèle : **non reproduite** en 3 exécutions ultérieures (2 complètes + 1 isolée), aucune assertion affaiblie.

**PR2q.2 — historique.** Le @pre-analyst a rendu **GO AVEC RÉSERVE** : `tauxEpargnePct` **n'existait nulle part dans le code applicatif** (absent de `prisma/schema.prisma`, `src/types/`, `previsions.schema.ts`, `parametres-tab.tsx` et des queries ; présent uniquement dans les fixtures du jeu d'or). Le pipeline a donc été **amendé par le chef de projet** par insertion d'une étape @db-specialist pour la migration. **Étape @db-specialist terminée** : `ParametresPrevision.tauxEpargnePct Decimal @default(30)` (échelle 0..100, NOT NULL, R7 justifié), migration `prisma/migrations/20260803170000_add_taux_epargne_pct/migration.sql` **idempotente** (garde `IF NOT EXISTS`), appliquée par `npx prisma migrate deploy` ; le scénario `EXCEL-V12` a reçu `tauxEpargnePct = 30` par **backfill du DEFAULT**, **aucune autre de ses données touchée**. Formule confirmée numériquement par le @pre-analyst sur les **21 mois des 2 fixtures**, écart **0,0** : `epargne(M) = max(0, resultat(M)) × tauxEpargne`. **Étape @developer terminée.**

**PR2q.3 — `FAIT`.** Pipeline exécuté intégralement (@pre-analyst → @developer → @tester → @code-reviewer). **@tester** (`docs/tests/rapport-story-PR2q3.md`) : recette rejouée **2 fois**, **1904/1904** (progression 1482 → 1904 confirmée exactement) ; suite complète rejouée 2 fois avant et 2 fois après ses ajouts (**278 fichiers passés / 5 skipped**, **8308 → 8313 tests**, **0 échec**) ; `npm run build` → **exit 0**. **Non-tautologie de la Section D confirmée par citation de code** : chaque valeur attendue provient d'un bloc de fixture (`entrees.*`, `besoinsAliments.*`, `depenses.alevinsCommandes`), jamais recalculée par le code testé. **Détection de régression prouvée** : injection d'un `+1` dans l'accumulation du `ceil` par granulométrie → **162 tests sur 1904 tombent** ; restauration vérifiée **octet pour octet** (diff/md5, le fichier n'étant pas suivi par git). Le **`ceil` par granulométrie** est bien appliqué **par granulométrie puis sommé**, jamais un ceil de la somme (vérification n°1 du README du jeu d'or). Les **lignes de granulométrie sont dynamiques** (union des clés réellement présentes, aucun 2/3/4 mm codé en dur) et réutilisent les libellés existants de `stock.json` — **aucun second référentiel**, conforme à ADR-053 §12.2.4. Les **deux sérialiseurs sont identiques champ pour champ**. La **réserve du @developer sur `resultats.totalEntrees` est confirmée exacte** : recherche exhaustive de `entreesModele` dans les deux fixtures — aucune entrée d'apport n'y existe comme donnée de saisie ; limite **documentée en JSDoc** dans le test plutôt que masquée. **5 tests de rendu de la carte mobile 375 px** ajoutés (seul le tableau desktop était exercé). Lignes **11-23 confirmées réellement exclues**, pas silencieusement omises. **@code-reviewer : `VALIDÉ AVEC RÉSERVES`** (`docs/reviews/review-story-PR2q3.md`), **aucune réserve Critique ni Haute** — (1) **Basse** : clé i18n morte `previsionsMensuellesTab.sectionToggleAria` (fr + en) jamais référencée, les boutons de bascule ayant **déjà** un nom accessible via leur texte visible (aucune régression d'accessibilité) — **soldée par le @developer de PR2q.4**, qui a supprimé la clé des deux langues ; (2) **documentaire** : le décompte « 8 → 30 lignes », **corrigé ici** en **8 → 19 lignes rendues** (dont 3 dynamiques), couvrant **17 des 30 lignes du classeur**.

**PR2q.3 — historique (étape @developer).** 6 nouveaux champs sur `MoisProjectionResult` / `MoisProjectionDTO` (`empoissonneKg`, `ventesKg`, `alevinsACommanderNb`, `besoinAlimentsTotalKg`, `sacsAlimentsTotal`, `sacsParGranulometrie`) et 4 accumulateurs mensuels dans `route-orchestration.ts` — **six séries étaient déjà calculées** mais gardées dans des variables locales de boucle ou des Map internes, jamais copiées dans la structure de sortie. Vue réécrite avec **4 sections repliables** (Radix Collapsible) : « Résultat » (dépliée par défaut : Total des entrées, Dépenses totales, Résultat du mois, Trésorerie cumulée), « Production », « Aliments », « Entrées & dépenses détaillées » (repliées) ; structure `LigneDescriptor` (`accessor`/`totalMode`/`format`), **aucune branche par nom de ligne**, lignes de granulométrie construites **dynamiquement**, `ExplicationLigne` sur toutes les nouvelles lignes. **8 → 19 lignes rendues** (dont **3 dynamiques** selon le nombre de calibres), couvrant **17 des 30 lignes du classeur** — lignes du classeur nouvellement exposées : 4, 5, 6, 7, 8-10, 24, 27, 33, 34 ; lignes 11-23 **exclues**. **Recette : 1482 → 1904 tests (+422), 0 écart**, via une « Section D » au grain `mois[]` : `empoissonneKg` ↔ `entrees.empoissonneT`, `ventesKg` ↔ `entrees.ventesT`, `besoinAlimentsTotalKg` ↔ `besoinsAliments.totalKg`, `sacsAlimentsTotal` ↔ `besoinsAliments.sacsTotal`, `sacsParGranulometrie.{G1,G2,G3}` ↔ `besoinsAliments.sacsParGranulometrie.{2mm,3mm,4mm}` (tolérance 0), `alevinsACommanderNb` ↔ `depenses.alevinsCommandes` (tolérance 0), `revenusFCFA` ↔ `entrees.chiffreAffaires` (≤ 1 FCFA). **Réserve déclarée par le @developer, à confirmer par le @tester** : la ligne 27 « Total des entrées » n'est **pas rapprochée intégralement** — `entreesModele` ne porte aucune entrée brute d'apports (`resultats.apportsCapital` est une **sortie** du classeur, jamais une ligne de saisie), donc le builder de recette construit toujours `apports: []` et la dérivation `revenusFCFA + apportsFCFA` dégénère à `revenusFCFA` ; documenté explicitement dans le JSDoc de la Section D plutôt que présenté comme couvert. Lignes **11-23 (détail par vague) non implémentées** — exclusion assumée. Suite complète après livraison : 283 fichiers (278 passed / 5 skipped), **8308 tests passés, 0 échec** ; `npm run build` compilé sans erreur.

**PR2q.3 — pré-analyse `GO`.** Points actés : le **détail par vague (lignes 11-23)** est **explicitement laissé hors périmètre**, sur recommandation assumée du @pre-analyst — `prisma/fixtures/previsions/extract-golden.py` ne lit pas ces lignes, et les exposer sans les extraire du classeur reviendrait à **comparer le moteur à lui-même**, l'anti-pattern que le README du jeu d'or interdit ; l'extraction est mécaniquement faisable et **mérite sa propre story**, pas un ajout silencieux. Les lignes **27 (Total des entrées)** et **33 (Résultat du mois)** sont dérivables côté UI sans toucher au moteur. La ligne **30 « Autres dépenses »** est déjà affichée **plus finement** par l'application (charges réparties et investissements en 2 lignes séparées) : à garder tel quel plutôt que fusionner.

**PR2q.4 — `FAIT`.** Pipeline exécuté intégralement (@pre-analyst → @developer → @tester → @code-reviewer). **@tester** (`docs/tests/rapport-story-PR2q4.md`) : recette **1904/1904 inchangée** ; suite complète rejouée **3 fois** (**8323/8323 à chaque run**) ; `npm run build` → **exit 0**. **Preuve de détection de régression** : retrait de l'exclusion `vaguePrevueId !== null` dans `ventilations.ts` → le test « **piège 2** » échoue **seul** (`expected 780000 to be 280000`), les **5 autres restent verts** — ce qui confirme leur **incapacité structurelle** à détecter ce bug, et donc la valeur du cas non dégénéré ; fichier restauré à l'identique, **checksum vérifié**. **@code-reviewer : `VALIDÉ AVEC RÉSERVES`** (`docs/reviews/review-story-PR2q4.md`), aucune réserve Critique ni Haute : (1) **Moyenne maintenue ouverte** — le filtre de `calculerBaseRepartition` est **dupliqué** entre `charges.ts` et `ventilations.ts`, un **ERR-138 en puissance** ; le test comparatif est un filet réel mais **partiel**, dont la valeur dépend de la discipline de maintenance future ; (2) **Basse** — cardinalité **non bornée** de la section Ventilations (une ligne par poste). L'absence d'injection HTML via `PostePrevision.libelle` a été vérifiée (interpolation `next-intl` puis rendu comme **enfant React**, jamais `dangerouslySetInnerHTML`).

**PR2q.4 — historique (étape @developer).** **Créé** : `src/lib/previsions/ventilations.ts` (logique pure : `ventilerApportsParType`, `ventilerDepensesParPoste`) et `src/lib/previsions/__tests__/ventilations.test.ts` (**6 tests**). 5ᵉ section « **Ventilations** », **repliée par défaut** (§7.1) ; props de `PrevisionsMensuellesTab` élargies, `scenario-detail-client.tsx` transmet `initialApports` / `initialCharges` / `initialPostes` / `initialJournal`, **déjà chargés — aucune nouvelle requête, aucun changement de moteur**. **Piège n°2 tranché** : la ventilation porte explicitement sur **`baseRepartitionFCFA`**, **pas** sur `depensesFCFA`, en **deux composantes reproduisant exactement le filtre de `calculerBaseRepartition`** — charges des `PostePrevision` à `inclusBaseRepartition = true` (groupées **par poste individuel** via `posteId` → `libelle`, jamais par `TypePostePrevision`, qui n'a que 2 valeurs) + journal `OPERATIONNEL` à `vaguePrevueId = null` (« Journal général ») ; les postes `inclusBaseRepartition = false` sont exclus, comme le moteur les exclut de `depensesFCFA` — les ventiler aurait créé un **écart silencieux**. **Garantie « total ventilé = ligne agrégée »** testée y compris dans le **cas non dégénéré** (journal `OPERATIONNEL` à `vaguePrevueId` non nul), que le jeu d'or **ne peut pas exercer** puisqu'il a ce journal nul partout sur les 21 mois des deux scénarios (ADR-053 §12.5, ERR-154). **Ce qui n'est pas rapprochable au jeu d'or, et pourquoi** : le classeur n'a qu'une ligne d'apports et une ligne de dépenses — la partition `CAPITAL`/`CREDIT` et le détail poste par poste **n'existent pas dans le classeur** ; seul le **total** est réconcilié (`resultats.apportsCapital` de `plan-v12-corrige.json`), `annexe-b-corrigee.json` étant **dégénéré** (0 apport partout) — documenté explicitement en tête des fichiers de test. **Dette soldée au passage** : clé i18n morte `sectionToggleAria` supprimée des **deux** langues. **Recette inchangée à 1904 tests, 0 écart** ; `npm run build` → **exit 0**. **Point ouvert à confirmer par le @tester** : échecs **flaky** sous exécution parallèle signalés par le @developer sur `aliment-form-dialog.test.tsx`, `apport-form-dialog.test.tsx`, `journal-form-dialog.test.tsx`, `poste-form-dialog.test.tsx`, `scenario-form-dialog.test.tsx`, `vague-prevue-form-dialog.test.tsx` et `render-pdf-safely.test.ts`, **tous passants isolément** (suite ciblée **1935/1935**) — à qualifier : instabilité de timing préexistante ou régression.

**PR2q.4 — pré-analyse `GO`.** Deux pièges actés : `TypePostePrevision` n'a que **2 valeurs** (`LOGISTIQUE`, `CHARGE_EXPLOITATION`), trop grossier — la ventilation doit **grouper par `PostePrevision` individuel** via `posteId` → `libelle`. Et `calculerBaseRepartition` **exclut les lignes de journal `OPERATIONNEL` affectées à une vague** : la ventilation doit porter sur **`baseRepartitionFCFA`**, pas sur `depensesFCFA` complet, ou inclure explicitement une catégorie pour le journal affecté — **à prouver par un test** avec au moins une ligne `OPERATIONNEL` + `vaguePrevueId` non nul (le jeu d'or a ce journal nul partout, il ne peut donc pas révéler le problème).

**PR2q.5 — `FAIT`.** Rapport : `docs/reviews/review-sprint-PR2-quinquies.md`. **Verdict de sprint : `VALIDÉ AVEC RÉSERVES`, aucune réserve Critique ni Haute.** Réserves priorisées : (1) `PalierRemise.seuilSacs` — **Moyenne**, héritée de **PR2-bis**, **inchangée**, **bloquante avant toute story de remise multi-granulométrie** ; (2) duplication du filtre `calculerBaseRepartition` — **Moyenne**, **bloquante avant toute story modifiant `base_repartition` dans `charges.ts`** ; (3) **garanties visuelles §7.4 hors de portée de jsdom** — **Moyenne, nouvelle** (capitalisée en **ERR-157**, sévérité Haute) ; puis **4 réserves Basses héritées** et **2 points d'information**. **Gouvernance PR2-bis confirmée toujours soldée** (grep de la première personne dans le fichier de sprint : **négatif**). **Périmètre tenu** (lignes 11-23 exclues, moteur non modifié hors PR2q.2). Capitalisation @knowledge-keeper : **ERR-153 à ERR-158** — *après la renumérotation opérée en PR2-septies (collision de numéros sur ERR-158), la capitalisation de ce lot couvre **ERR-153 à ERR-158 plus ERR-163***.

**Correctif post-livraison — `FAIT` (hors périmètre initial).** Une vérification de **rendu réel** (Chromium/Playwright, scénario `EXCEL-V12`, **1280×800** et **375×812**) a révélé un défaut **qu'aucun test jsdom ne pouvait voir** : les **titres de section disparaissaient au défilement horizontal**. Ils étaient un `<td colSpan={mois.length + 2}>` en `sticky left-0` — une cellule occupant **déjà toute la largeur du tableau** n'a **aucune marge pour s'épingler** (mesuré : titre à `left = -1331` pour un conteneur commençant à 264, soit **1595 px hors écran** ; les 5 en-têtes devenaient des **bandes grises vides**). **Correctif** : cellule collante **étroite** (largeur de la colonne des libellés, fond `bg-muted` **opaque**, `z-10`, `border-r`) + bande de fond **non collante** en `colSpan={mois.length + 1}` avec `aria-hidden="true"` — **réutilisation exacte** du mécanisme déjà en place pour les lignes d'indicateurs, **aucun mécanisme parallèle** ; plus deux correctifs cosmétiques (`text-left` sur le bouton de bascule mobile, que le style UA centrait, et `collisionPadding = 16` par défaut sur `PopoverContent`). **Revérifié en navigateur réel après correctif** : titre mesuré à `265 → 717` à `scrollLeft` **0, 804, 1608 et max** — identique à la colonne des libellés, **jamais déplacé**, pour les **5 sections** ; fond `rgb(241, 245, 249)` **sans canal alpha**, donc réellement opaque ; `elementFromPoint` renvoie le bouton de section à 15/40/60/85 % de la largeur de la cellule. En **375 px** : les 5 boutons alignés à gauche (texte à `x = 55`), « VENTILATIONS » sur 2 lignes sans se recentrer ; marge du popover portée à **16 px** (`right = 359` dans un viewport de 375, contre 375 auparavant). **Non-régression confirmée** : première colonne collée à `265 → 717`, **23 en-têtes** atteignables, `TOTAL` visible à scroll max, **aucun débordement de page** (`scrollWidth = clientWidth = 1280` et `= 375`), **22 lignes** après dépliage, **22/22 popovers desktop** s'ouvrent, formats **§7.4 vérifiés au caractère près** (381 espaces fines U+202F, 150 tirets U+2013, `rgb(239, 68, 68)` sur les négatifs, tonnages `4,0 t`). **3 tests de non-régression jsdom** ajoutés, avec mention **explicite dans le test** de ce qu'ils **ne peuvent pas** prouver.

**Vérification de fin de sprint — sorties réelles** (`docs/tests/rapport-final-sprint-PR2-quinquies.md`) :
- `npx prisma migrate deploy` : **aucune migration en attente**.
- Recette : **1904/1904, 0 écart** (461 + 461 + 982). Progression du sprint : **1270 → 1482 → 1904**.
- Suite complète, **3 runs identiques** : **284 fichiers** (279 passed + 5 skipped), **8326 tests passed**, 21 skipped, 26 todo, **0 échec aux 3 runs**.
- `npm run build` : `✓ Compiled successfully`.
- Fixtures du jeu d'or **intactes** (`git status` / `git diff --stat` vides).
- Scénario `EXCEL-V12` vérifié **en lecture seule**, conforme : 19 vagues, 602 500 alevins, 3 calibres G1/G2/G3, 241 t, `tauxEpargnePct = 30` — **aucune de ses données modifiée** pendant le sprint.
- Parité i18n : **407 clés de chaque côté**, aucune manquante ; `sectionToggleAria` bien disparue des deux fichiers ; **aucune clé morte nouvelle** (seules `page.detailTitle` / `page.backToList`, connues et hors périmètre, subsistent — le critère « clés mortes absentes des deux langues » de PR2q.5 reste donc **partiellement satisfait**, laissé décoché dans le fichier de sprint).
- Vérification manuelle : faite **en navigateur réel** à **375×812** et **1280×800** (pas à 360 px), par **mesure** et non à l'œil.
- Les **3 cassages volontaires** des campagnes de test (`calculerEpargne`, le `ceil` par granulométrie dans `route-orchestration.ts`, le filtre de `ventilerDepensesParPoste`) sont confirmés **restaurés correctement**, par lecture de code **et** par les tests passants.
...
5. **Nouvelles entrées de capitalisation** : **ERR-153 à ERR-158** ajoutées à `docs/knowledge/ERRORS-AND-FIXES.md`, dont **ERR-157** (sévérité **Haute**) : *jsdom ne prouve aucune garantie de mise en page*.
6. **Critère laissé décoché en PR2q.2** : `tauxEpargnePct` « lisible et modifiable dans les paramètres du scénario » — la **persistance** est acquise (colonne, migration, transit DB → loader → moteur prouvé par test e2e), mais **aucun rapport du sprint n'atteste de l'exposition du champ dans l'UI `parametres-tab.tsx`**, que la pré-analyse signalait absente. À confirmer ou à traiter en story dédiée.

---

## Sprint PR2-sexies — Détail des sacs par mois de cycle
Voir : [SPRINT-PR2-sexies-PREVISIONS.md](sprints/SPRINT-PR2-sexies-PREVISIONS.md)

**Statut :** `FAIT` (**4 stories** — toutes `FAIT`) — **review de sprint `VALIDÉ AVEC RÉSERVES`, aucune réserve Critique ni Haute ouverte**
**Objectif :** le classeur `Previsions_Elevage_Silure_v12.xlsx` porte, **lignes 11 à 23** de sa feuille `Prévisions`, un bloc **« DÉTAIL PAR VAGUE — sacs consommés dans le mois (indicatif) »** : les sacs consommés chaque mois, ventilés **par position de la vague dans son cycle** (1er, 2e, 3e mois) **ET** par **granulométrie** (2 mm, 3 mm, 4 mm) — soit **9 séries**. **L'application n'en expose aucune.** PR2-quinquies les avait **délibérément écartées** parce que `prisma/fixtures/previsions/extract-golden.py` ne les extrayait pas, et que les exposer sans jeu d'or reviendrait à **comparer le moteur à lui-même**. **Ce sprint lève ce blocage** : étendre l'extraction d'abord, exposer ensuite.
**Formules du classeur** (granulométrie `g`, mois `M` ; `sacs(g, vague)` = `'Aliment par vague'` colonnes **D/E/F** ; `pct_mois_k(g)` = `Aliments` colonnes **F/G/H**) :
```
...
```
**⚠️ POINT D'ATTENTION :** c'est un **`ROUND`**, **PAS** le **`ceil`** de la ligne « Sacs à acheter ». Le classeur qualifie ce bloc d'**« indicatif »** : ce sont les sacs **CONSOMMÉS**, pas les sacs **À COMMANDER**. **Confondre les deux arrondis est exactement le glissement qui a produit le bug de facteur 8300** (ADR-053 §11, **ERR-138/ERR-139**).
...
**Défaut du classeur à NE PAS reproduire :** ses lignes **12, 16 et 20** affichent un code de vague obtenu par **`INDEX/MATCH`**, qui **ne renvoie qu'UNE seule vague** même quand plusieurs coïncident (signalé au **§8 des exigences** comme un défaut à corriger). Les **quantités**, elles, utilisent **`SUMIFS`** et **cumulent correctement** — **c'est ce comportement-là qu'il faut implémenter**. Si un libellé de cohorte est exposé, il doit lister **TOUTES** les vagues concernées.

| Story | Type | Sujet | Pipeline | Statut |
|-------|------|-------|----------|--------|
| PR2sex.1 | TEST | Étendre `extract-golden.py` aux 9 séries « détail par vague » (lignes 13-15, 17-19, 21-23 de la feuille `Prévisions`, colonnes B à V) + régénérer les 2 fixtures + prouver l'invariance des séries existantes + MAJ README | @tester | `FAIT` |
...
**PR2sex.1 — `FAIT`.** Nouvelle fonction `extraire_detail_par_vague(pv)` dans `prisma/fixtures/previsions/extract-golden.py`, injectée sous la clé **`besoinsAliments.detailParVagueSacs`** dans les deux fixtures. La correspondance ligne↔série a été confirmée par **lecture directe des libellés de la colonne A** du classeur (lignes 11-24), et non par déduction : 13/14/15 = 1er mois de cycle 2/3/4 mm, 17/18/19 = 2e mois, 21/22/23 = 3e mois. Les lignes **12/16/20** (INDEX/MATCH défectueux, §8) sont extraites **uniquement comme métadonnée `$defectueux`**, jamais comme série numérique. Les **9 cumuls sur 21 mois** tombent **exactement** sur les valeurs de contrôle attendues (1543/867/0 · 385/3471/4820 · 0/0/7230), **aucun écart**. **Preuve d'invariance** : diff structurel clé par clé avant/après régénération = **0 clé supprimée, 0 valeur changée, 1 seule clé ajoutée par fixture** ; **idempotence confirmée** (sha256 identiques sur deux exécutions). Les séries ne sont **affectées ni par le patch `Dépenses!B10` ni par le scénario B** (ce sont des **décomptes de sacs, pas des montants**) — confirmé par **identité stricte du bloc** entre les fixtures A et B. **Vérifications** : `npx vitest run` = **286 fichiers / 8333 tests / 0 échec** ; recette = **1904 tests / 0 écart** (inchangée, moteur non touché) ; `npm run build` **OK**. **Rapport** : `docs/tests/rapport-story-PR2sex.1.md`.

**PR2sex.2 — `FAIT`.** Pipeline exécuté intégralement (@pre-analyst → @developer → @tester → @code-reviewer → @knowledge-keeper). Fonction pure `calculerDetailConsommationMensuelle` en fin de `src/lib/previsions/aliments.ts` (+ types `DetailConsommationCycleInput` / `DetailConsommationMoisResult`), strictement **additive** : les **6 signatures gelées** de l'ADR-053 §12.4 sont **intactes**. **sum-then-round** : sommation dans l'accumulateur `sacsEffectifsCycleParAlimentMoisPosition` (`route-orchestration.ts:372`, alimenté l. 519-530), **round unique par cellule** (l. 672), `Decimal.ROUND_HALF_UP` **explicite** (half-away-from-zero, comme le `ROUND` d'Excel). Ce choix a été établi par **lecture de la formule brute** du classeur (`Prévisions!B13`), **PAS** par la recette : les 19 vagues ont **19 mois d'empoissonnement distincts**, donc le jeu d'or **ne peut structurellement pas** discriminer `ROUND(Σ)` de `Σ ROUND()` (**ERR-160**) — d'où des **tests synthétiques** qui font **réellement diverger** les deux candidats, au niveau de la fonction pure **et** de l'accumulateur (`route-orchestration-detail-consommation.test.ts`, **5 tests**). **Non-contamination `ceil`/`ROUND` prouvée par construction** (101 sacs à 33 % → **33** et non 34) — **ERR-161**. **Cycle paramétrable** : aucun `3` en dur, `dureeCycleMoisFigee` partout, tests à `dureeCycleMois = 4` et `= 1`. **Exposition** : `MoisProjectionResult.detailParVagueSacs: Record<number, Record<string, number>>` → `MoisProjectionDTO` → route `GET /api/previsions/scenarios/[id]/calculer`. **Recette : 2300 tests, 0 écart** (1904 + 378 Section E + 18 assertions de cumul sur l'horizon, contre 1543/867/0 · 385/3471/4820 · 0/0/7230). **Review** (`docs/reviews/review-story-PR2sex.2.md`) : **VALIDÉ AVEC RÉSERVES**, **aucune Critique ni Haute** — 1 réserve **Moyenne** (**ERR-162**, ouverte), 3 **Basses**, 1 **Info**. **Franchise ERR-155** : **3 des 9 séries** (`moisCycle1.4mm`, `moisCycle3.2mm`, `moisCycle3.3mm`) sont **entièrement nulles** dans les deux fixtures — **42 assertions `0 == 0`**, documentées comme ne prouvant presque rien. **Rapports** : `docs/tests/rapport-story-PR2sex.2.md`, `docs/analysis/pre-analysis-story-PR2sex.2.md`.

**PR2sex.3 — `FAIT`.** Sous-section « **Détail par mois de cycle** » **repliée par défaut** sous « Aliments », via `SectionDescriptor.groupes?: SousSectionDescriptor[]` ; le motif d'en-tête collant à **2 cellules** (fix **ERR-157**) est **extrait** dans un composant partagé `EnTeteRepliable`, **jamais réimplémenté** ; rendu de ligne extrait dans `LigneRow`. Lignes produites par **produit cartésien** des positions de cycle et des granulométries **réellement présentes**, **jamais `1,2,3` en dur**. **i18n fr/en**, parité stricte **406/406**, clés mortes `page.detailTitle` et `page.backToList` **supprimées des deux langues**. **Vérification Chromium réelle (Playwright, ERR-157)** : en-tête de sous-section **stable** entre `scrollLeft = 0` et `scrollLeft = 1760` à **375 px**, et entre `0` et `1399` à **1280 px** ; **aucun débordement de page** (`scrollWidth === clientWidth === 375`) ; fond **opaque** `rgb(241, 245, 249)` ; colonne Total **lue à l'écran** = 1 543 / 867 / – puis 385 / 3 471 / 4 820, **conforme au jeu d'or**. **@tester : verdict PASS** (`docs/tests/rapport-story-PR2sex.3.md`). **Non vérifié et déclaré comme tel** : cumul de la position 3 en pixels réels, collision de popover à 375 px, rendu tactile. **Review** (`docs/reviews/review-story-PR2sex.3.md`) : **VALIDÉ AVEC RÉSERVES**, **aucune Critique ni Haute** — **1 Moyenne de signalement** (flakiness de la suite, **hors périmètre**), **4 Basses**, **1 Info**. Le **motif ERR-157 à 2 cellules** est **préservé à la lettre** dans `EnTeteRepliable` (cellule collante **étroite sans `colSpan`**, fond **opaque** `bg-muted`, bande `colSpan` en `aria-hidden`), **réutilisé à l'identique aux deux niveaux**. **Aucune valeur en dur** : grep **négatif** sur `1,2,3` et sur `2mm` / `3mm` / `4mm`. **Parité i18n stricte 406/406** (405 avant + **3 ajoutées** − **2 mortes supprimées**).

**PR2sex.4 — `FAIT`.** Rapport : `docs/reviews/review-sprint-PR2-sexies.md` — **verdict `VALIDÉ AVEC RÉSERVES`, aucune réserve Critique ni Haute ouverte**.

**Vérification de fin de sprint — mesures réelles :**
- **Données utilisateur `EXCEL-V12` INTACTES** (lecture seule stricte respectée) : **19 vagues**, **602 500 alevins**, **3 apports**. *Note méthodologique* : le scénario s'identifie par `ScenarioPrevision.code = 'EXCEL-V12'` — son champ `nom` vaut « Plan de reference Excel v12 » ; une requête sur `nom` renvoie **0 à tort**.
- Recette : `npx vitest run src/lib/previsions/__tests__/recette` → **2300 / 2300, 0 écart** (461 + 461 + 1378).
- Suite complète : **287 fichiers / ~8800 tests**. Deux passages : **8733 passés / 20 échoués**, puis **8726 passés / 27 échoués**. **11 fichiers en échec à chaque passage, composition variable** ; les **12 fichiers distincts** concernés ont été **rejoués isolément — 12/12 passent à 100 %**. Cause : `Error: Test timed out in 5000ms` sous **contention CPU en exécution parallèle**, **jamais une `AssertionError` métier** ; concerne notamment les `*-form-dialog.test.tsx`, `password.test.ts` (bcrypt), et **par ricochet** `previsions-mensuelles-tab.test.tsx` et `scenario-detail-client-refresh.test.tsx`.
...
- **i18n** : **406 clés de chaque côté**, aucune clé d'un seul côté ; `page.detailTitle` et `page.backToList` **absentes des deux fichiers** et **sans référence dans `src/`**.
- **Aucun commit, aucun push** — conformément à la gouvernance.

**Points ouverts consignés (PAS des stories de ce sprint) :**
...
2. **Dette de fiabilité de la suite de tests** — mesures réelles de clôture : **11 fichiers en échec à chaque passage**, **20 tests** au 1er passage et **27** au 2e, **composition variable** ; les **12 fichiers distincts** concernés sont **verts en isolation (12/12)**. Cause : `Error: Test timed out in 5000ms` sous **contention CPU en exécution parallèle**, **jamais une `AssertionError` métier**. **Piste** : ajuster le `testTimeout` global ou la **concurrence** vitest. Pas une régression métier, mais la suite n'est plus un signal binaire fiable.
3. **Collision de numérotation ERR-158** — **deux entrées distinctes** portent ce numéro dans `docs/knowledge/ERRORS-AND-FIXES.md`. **Non corrigée** à la clôture de ce sprint, car des références existent déjà dans `docs/TASKS.md`, `docs/sprints/SPRINT-PR2-quinquies-PREVISIONS.md` (plage « ERR-153 à ERR-158 ») et `docs/analysis/pre-analysis-story-PR2sex.3.md`. **➜ Soldé en PR2-septies** : l'**ancienne première ERR-158** a été **renumérotée en ERR-163**.
...
5. **ERR-160 (Haute, documentée)** — le jeu d'or est **structurellement aveugle à l'ordre d'agrégation** (`ROUND(Σ)` vs `Σ ROUND()`) : **19 vagues**, **19 mois d'empoissonnement distincts**, **aucune coïncidence**. La garantie repose **exclusivement** sur la lecture de la **formule brute** du classeur et sur **5 tests synthétiques dédiés**. **Si ces tests étaient affaiblis ou supprimés, plus aucune protection ne subsisterait**, malgré **2300 tests de recette verts**.
6. **Correction du chiffre de référence de la suite** — **286 fichiers / 8333 tests → 287 fichiers / ~8800 tests** : la base annoncée en ouverture de sprint était antérieure au module Prévisions complet. À reprendre comme référence dans les sprints suivants.

**Contraintes transverses :** i18n `fr` et `en` complètes, accents corrects, aucune chaîne en dur, clés mortes supprimées dans les deux langues (dont les **2 clés mortes résiduelles** `page.detailTitle` et `page.backToList` signalées par la review PR2-quinquies) ; **formats du §7.4 conservés** (espace fine **U+202F**, aucune décimale sur les montants, zéros en tiret **U+2013**, négatifs en rouge, unité dans le libellé de ligne, bouton d'explication sur chaque ligne calculée) ; le tableau mensuel vient d'être **transposé** (indicateurs en lignes, mois en colonnes), rendu **défilable** et **unifié mobile/bureau** : **CONSTRUIRE DESSUS, ne rien défaire** ; le moteur `src/lib/previsions/` n'est modifiable que par **PR2sex.2** ; **recette exigée ≥ 1904 tests / 0 écart**, tolérance **0** (entiers) sur **21 mois × 3 granulométries × 3 positions de cycle**, sur les **DEUX** fixtures ; **cycle paramétrable** — boucler sur `k = 1..dureeCycleMois`, **jamais trois branches en dur** (le jeu d'or a un cycle de 3, le code ne doit pas le supposer) ; **fixtures** : valeurs lues dans les **CELLULES CALCULÉES** du classeur, **jamais recalculées**, invariance des séries déjà présentes **à prouver** (comparaison avant/après par **empreinte**) ; **ERR-157** — les garanties visuelles ne se prouvent pas en jsdom, donc **vérification en navigateur réel à 375 px et 1280 px** ; **lecture seule stricte** sur le scénario `EXCEL-V12` (jeu de test de l'utilisateur, **en cours d'utilisation**) : vérifier **par SQL en fin de sprint** que **19 vagues / 602 500 alevins / 3 apports** sont **inchangés**.

**Vérification de fin :** `npx vitest run` (base : **286 fichiers, 8333 tests hors DB-gated, 0 échec**) ; recette `src/lib/previsions/__tests__/recette` **≥ 1904 / 0 écart** ; `npm run build`.

**Gouvernance :** seul le @status-updater écrit dans `docs/sprints/` et `docs/TASKS.md`. **Aucun commit ni push par les agents.**

**Hors périmètre :** toute modification du classeur source ; la partition **CAPITAL/CREDIT** des apports ; les réserves ouvertes de la review PR2-quinquies (`PalierRemise.seuilSacs`, duplication de `calculerBaseRepartition`) — **sauf si elles bloquent**.

**⚠️ Identifiants de stories :** ce sprint utilise le préfixe **`PR2sex.`** et **non** `PR2q.`, les identifiants `PR2q.1` à `PR2q.6` étant **déjà utilisés deux fois** (PR2-quater **et** PR2-quinquies) — collision documentée dans le fichier de sprint PR2-quinquies.

---

## Sprint PR2-septies — La remise fournisseur se décide au tonnage de la vague
Voir : *aucun fichier `docs/sprints/SPRINT-PR2-septies-*.md` n'existe* — le suivi de ce sprint est porté ici, dans `docs/TASKS.md`. Livrables : [ADR-053 §13](decisions/ADR-053-module-previsions.md), [pre-analysis-sprint-PR2-septies.md](analysis/pre-analysis-sprint-PR2-septies.md), [rapport-story-PR2sept.3.md](tests/rapport-story-PR2sept.3.md), [rapport-sprint-PR2-septies.md](tests/rapport-sprint-PR2-septies.md), [review-sprint-PR2-septies.md](reviews/review-sprint-PR2-septies.md).

**Statut :** `FAIT` (**5 stories** — toutes `FAIT`) — **review de sprint `VALIDÉ AVEC RÉSERVES`, aucune réserve bloquante**
**Objectif :** corriger **ERR-143**, signalée **bloquante depuis deux sprints** (réserve **Moyenne** héritée de **PR2-bis**, reconduite par les reviews de sprint PR2-quinquies et PR2-sexies) : `PalierRemise.seuilSacs` fait dépendre la remise fournisseur d'un **nombre de sacs**, alors que la remise **se décide au tonnage de la vague**. Le seuil devient `PalierRemise.seuilTonnes`, le moteur décide la remise **par vague sur le tonnage**, et les **3 contournements de recette** introduits pour vivre avec l'ancien modèle sont **supprimés**.

**Pré-analyse commune aux stories PR2sept.2, PR2sept.3 et PR2sept.4 : `FAITE`** — `docs/analysis/pre-analysis-sprint-PR2-septies.md`, verdict **GO sur les trois**.

| Story | Type | Sujet | Pipeline | Statut |
|-------|------|-------|----------|--------|
| PR2sept.1 | ADR | Amendement ADR-053 §13 — la remise se décide au tonnage de la vague | @architect seul | `FAIT` |
| PR2sept.2 | SCHEMA | `PalierRemise.seuilSacs` → `seuilTonnes` (schéma + migration) | @pre-analyst → @db-specialist → @code-reviewer → @knowledge-keeper | `FAIT` |
| PR2sept.3 | QUERIES | Moteur — remise décidée par vague sur le tonnage, suppression des 3 contournements de recette | @pre-analyst → @developer → @tester → @code-reviewer → @knowledge-keeper | `FAIT` |
| PR2sept.4 | UI | Libellé du seuil en tonnes, aide contextuelle, i18n fr/en | @pre-analyst → @developer → @tester → @code-reviewer | `FAIT` |
| PR2sept.5 | REVIEW | Review de sprint → `docs/reviews/review-sprint-PR2-septies.md` | @code-reviewer → @knowledge-keeper | `FAIT` |

**PR2sept.5 — `FAIT`.** Rapport : `docs/reviews/review-sprint-PR2-septies.md` — **verdict de sprint `VALIDÉ AVEC RÉSERVES`, aucune réserve bloquante**.

**ERR-143 est close** (`CORRIGÉ / FERMÉ`), sa sévérité ayant été **relevée de Moyenne à Haute** : la règle du **§4.3 des exigences** est désormais **exprimable depuis l'interface**. Migration livrée : `prisma/migrations/20260804100000_palier_remise_seuil_tonnes/`.

**Vérification de fin de sprint — mesures réelles :**
- `npx prisma migrate deploy` : **aucune migration en attente**.
- `npx vitest run` : **3 passages consécutifs, compteurs identiques, 0 échec** — **288 fichiers, 8 865 tests**.
- `npm run build` : **OK**.
- Recette : **2 378 assertions / 0 écart**.
- **Données utilisateur `EXCEL-V12` INTACTES**, vérifié **par SQL** : **19 vagues**, **602 500 alevins**, **3 apports**, `updatedAt = 2026-08-03 20:10:26.493` **inchangé**. La **seule écriture en base** de ce sprint est le `RENAME COLUMN` + l'index unique, sur une table `PalierRemise` **vide (0 ligne)**.
- **Réserve M-2 de la review levée après sa rédaction** : la vérification **360 px en Chromium réel** a bien été faite par @developer lors de la passe de correction finale (**360 px et 1280 px**, `scrollWidth = clientWidth = 360`, **aucun débordement**, message d'erreur correctement positionné sous le champ). Elle **ne reste pas ouverte**.

**Livrables :** `docs/decisions/ADR-053-module-previsions.md` **§13.1-13.8**, `docs/analysis/pre-analysis-sprint-PR2-septies.md`, `docs/tests/rapport-story-PR2sept.3.md`, `docs/tests/rapport-sprint-PR2-septies.md`, `docs/reviews/review-sprint-PR2-septies.md`.

**Capitalisation @knowledge-keeper :** **ERR-143 réécrite et close** ; fiches créées **ERR-164** (parité i18n ≠ justesse d'un libellé), **ERR-165** (couplage HTTP par sous-chaîne d'un message utilisateur), **ERR-166** (`aria-describedby` orphelin, latent dans `textarea.tsx` / `select.tsx`), **ERR-167** (79 warnings Radix `Missing Description`), **ERR-168** (pratique : validation par falsification), **ERR-169** (note de correspondance ADR). L'**ancienne première ERR-158** a été **renumérotée en ERR-163**, soldant la collision de numéros consignée à la clôture de PR2-sexies.

**Points ouverts consignés — à porter en backlog (PAS des stories de ce sprint, aucun bloquant) :**

| Sévérité | Point |
|---|---|
| Moyenne | Refonte de `PREVISIONS_STATUS_MAP` vers une erreur métier typée (`BusinessRuleError { status }`) — signalée par **3 reviews consécutives** (**ERR-165**) |
| Basse | `textarea.tsx` / `select.tsx` : même défaut `aria-describedby` orphelin, **latent** (**ERR-166**) |
| Basse | **79 warnings** Radix `Missing Description for DialogContent` sur **47 fichiers de test** (**ERR-167**) |
| Basse | `validation.ts:68` : sortir `seuilTonnes=` du message utilisateur (camelCase dans une UI française) |
| Basse | `previsions.schema.ts:38` : renommer `positiveInt` → `nonNegativeInt` (le nom ment, c'est `.nonnegative()`) |
| Basse | `api-utils.ts` : JSDoc inexacte sur la priorité P2002 / `statusMap` |
| Basse | `previsions-scenarios.ts:503` : ajouter `siteId` au `deleteMany` (défense en profondeur) |
| Dette acceptée | Course entre deux `PUT` concurrents sur les paliers (P2002 possible, **aucune corruption**, réessai suffit) |
| Pour @architect | Amender `ADR-053:1973` : `mettreAJourPaliersRemise` **n'existe pas**, la fonction est `replacePaliersRemise` |
...
| # | Réserve |
|---|---|
| 1 | README des fixtures corrigé — **fait** (« 6 postes non nuls » → « 4 postes non nuls (sur 8 lignes, dont 4 à zéro) ») |
| 2 | Faire **confirmer par l'exploitant** les deux libellés « Vente Vague 26-03 » / « Vente Vague 26-04 » rangés en **apports en capital** |
...
## Sprint PR2-octies — Alevins achetés ou produits en interne
Voir : *aucun fichier `docs/sprints/SPRINT-PR2-octies-*.md` n'existe* — le suivi de ce sprint est porté ici, dans `docs/TASKS.md`.

**Statut :** `FAIT` (**5 stories** — toutes `FAIT`) — **review de sprint `VALIDÉ`, aucun constat Critique ni Haute**
**Objectif :** le **§4.3 des exigences** définit sur chaque vague `alevins_achetes` (`false` = **production interne** → coût **0**) et le **§5.3** pose `cout_alevins(vague) = alevins_achetes ? nb_alevins × prix_alevin × (1 − remise) : 0`. Ce drapeau **n'existe nulle part** : le moteur **facture toujours** les alevins. Le **jeu d'or** porte pourtant `alevinsAchetes: NON` sur ses **19 vagues**, `coutAlevinsFCFA` total = **0**, alors que `prixAlevinUnitaireFCFA` vaut **70**. Écart en jeu : **environ 46 millions FCFA**.

| Story | Type | Sujet | Pipeline | Statut |
|-------|------|-------|----------|--------|
| PR2oct.1 | ADR | Amendement ADR-053 §14 — les alevins ne sont pas toujours achetés | @architect seul | `FAIT` |
| PR2oct.2 | SCHEMA | `VaguePrevue.alevinsAchetes` + `ParametresPrevision.alevinsAchetesParDefaut` + migration | @pre-analyst → @db-specialist → @code-reviewer → @knowledge-keeper | `FAIT` |
| PR2oct.3 | API | Moteur — `cout_alevins` suit le §5.3, logistique alevins indépendante du drapeau | @pre-analyst → @developer → @tester → @code-reviewer → @knowledge-keeper | `FAIT` |
| PR2oct.4 | UI | Drapeau visible et modifiable sur la vague prévue, défaut dans les paramètres, i18n fr/en | @pre-analyst → @developer → @tester → @code-reviewer | `FAIT` |
| PR2oct.5 | REVIEW | Review de sprint → `docs/reviews/review-sprint-PR2-octies.md` | @code-reviewer → @knowledge-keeper | `FAIT` |

**PR2oct.1 — `FAIT`.** Amendement `ADR-053 §14` — « les alevins ne sont pas toujours achetés » : le manque et pourquoi la recette ne l'attrapait pas (**§14.1**), le couple `VaguePrevue.alevinsAchetes` / `ParametresPrevision.alevinsAchetesParDefaut` calqué sur le patron `effectifAlevinsParVague` → `effectifAlevinsPrevu` (**§14.2**), le défaut **`false`** justifié par le jeu d'or (**§14.3**), `prixAlevinUnitaireFCFA` **toujours conservé** (**§14.4**), la mise en garde « **la logistique alevins ne dépend PAS du drapeau** » (**§14.5**), la nécessité d'une **couverture synthétique du cas `true`** (**§14.6**) et la **réserve sur la cécité de la recette** (**§14.7**). **Aucune contradiction relevée** avec les §11-13.

**PR2oct.2 — `FAIT`.** `prisma/schema.prisma` : `VaguePrevue.alevinsAchetes Boolean @default(false)` et `ParametresPrevision.alevinsAchetesParDefaut Boolean @default(false)`, **tous deux NOT NULL (R7)**. Migration versionnée idempotente `prisma/migrations/20260805090000_add_vague_prevue_alevins_achetes/migration.sql` (**R10**) : `ADD COLUMN` sous `DO $$ IF NOT EXISTS`, plus un `UPDATE` **ciblé et no-op silencieux** restaurant `prixAlevinUnitaireFCFA = 70` sur le seul scénario `EXCEL-V12`. **Miroir TypeScript complété** dans `src/types/models.ts` après la review. Review : `docs/reviews/review-story-PR2oct.2.md` — verdict **`VALIDÉ AVEC RÉSERVES`**, **les deux réserves ont été levées** (**R3** : miroir TypeScript complété par @architect ; **capitalisation** : entrée **ERR-170** ajoutée à `docs/knowledge/ERRORS-AND-FIXES.md` par @knowledge-keeper).

**Protection des données utilisateur — scénario `EXCEL-V12`, mesures réelles AVANT/APRÈS la migration :** `VaguePrevue` **19 lignes** / `SUM(effectifAlevinsPrevu)` = **602 500** — **inchangé**, et **0/19** vagues à `alevinsAchetes = true` (conforme au jeu d'or) ; `ApportCapital` **3 lignes** — **inchangé** ; `AlimentPrevision` **3 calibres**, 1 article et 3 répartitions chacun — **inchangé** ; `ScenarioPrevision.updatedAt` = `2026-08-03 20:10:26.493` — **inchangé** ; `ParametresPrevision` **colonne par colonne** — **une seule différence : `prixAlevinUnitaireFCFA` 0 → 70**, restauration **explicitement autorisée et documentée**, plus l'ajout attendu de `alevinsAchetesParDefaut = false`. **Aucune régression détectée.** Snapshot : `docs/analysis/snapshot-avant-sprint-PR2-octies.md`.

**Pré-analyses `FAITES` :** `docs/analysis/pre-analysis-story-PR2oct.2.md` (**GO avec réserves**), `docs/analysis/pre-analysis-story-PR2oct.3.md` (**GO**), `docs/analysis/pre-analysis-story-PR2oct.4.md` (**NO-GO en l'état** — la conception UI est prête mais **dépend de la livraison de PR2oct.3**, notamment de l'extension des schémas zod et des DTO).

**Constat consigné avant travaux :** sur le scénario `EXCEL-V12` en base, `ParametresPrevision.prixAlevinUnitaireFCFA` vaut **déjà 0** alors que le classeur de référence porte **70** — **dommage antérieur à ce sprint**, à **restaurer par migration idempotente ciblée**.

**PR2oct.3 — `FAIT`.** Le moteur applique désormais `coutAlevinsFCFA = vague.alevinsAchetes ? Decimal(alevinsACommanderNb) × prixAlevinUnitaireFCFA : Decimal(0)` (`src/lib/previsions/route-orchestration.ts`), avec `nb_alevins` = `alevinsACommanderNb` (marge de sécurité incluse) et `Decimal` préservé de bout en bout. Propagation du défaut de scénario aux **quatre** points d'écriture de `src/lib/queries/previsions-vagues.ts` : `createVaguePrevue` (défaut de scénario), `updateVaguePrevue` (édition libre), `scinderVaguePrevue` (**copie du parent**, jamais le défaut de scénario), `genererPlanVaguesPrevues` (défaut de scénario au `createMany`). Schémas zod, DTO, loader et `api-types.ts` étendus de bout en bout.

**`logistique.ts` volontairement intouché** — mise en garde ADR-053 §14.5 : le transport des alevins ne dépend **pas** du drapeau. Fait vérifié contre les fixtures : `voyagesAlevins`/`transportAlevins` sont non nuls sur **19 des 21 mois** du jeu d'or alors que les 19 vagues sont en production interne. Une ferme qui produit ses alevins les transporte quand même. Le cas synthétique D le prouve chiffré : `voyagesAlevins = 1`, `sousTotalFCFA = 45 000` alors que le coût d'achat vaut 0 le même mois.

**Le filet — la cécité de recette est comblée.** La recette ne comparait **jamais** `coutAlevinsFCFA` à `fixture.depenses.alevins` : environ **42 175 000 FCFA** d'écart qu'aucun test ne pouvait attraper (ERR-170). **+80 assertions** ajoutées (19×2 sur les vagues, 21×2 sur les mois) : recette **2 378 → 2 458 assertions, 0 écart**. **Preuve de discrimination** : en revenant temporairement à l'ancienne formule non gatée, **76 des 80 assertions échouent** (écarts jusqu'à **2 887 500 FCFA**), fichier ensuite restauré à l'identique (`git diff --stat` vide).

**Couverture synthétique du cas `alevinsAchetes = true`**, que le jeu d'or n'exerce jamais : `src/lib/previsions/__tests__/route-orchestration-alevins-achetes.test.ts`, 4 cas à montants écrits en dur et calculés à la main — cas A `ceil(10 000 × 1,10) = 11 000` alevins × 70 FCFA = **770 000 FCFA**, recalculé indépendamment par le @code-reviewer et confirmé exact. Plus **9 tests** de propagation du défaut dans `src/lib/queries/__tests__/previsions-vagues.test.ts`.

**Réserve tranchée, à consigner en backlog :** le §5.3 des exigences écrit `× (1 − remise)` mais **aucune remise alevins n'est modélisée** — le `PalierRemise` au tonnage est explicitement borné au coût des aliments (ADR-053 §13.3). Le choix assumé est de **ne pas en inventer** : aucune donnée de recette ne permettrait de discriminer une formule candidate d'une autre. Si un futur besoin exige une remise alevins, il faudra **d'abord** une donnée de recette qui l'exerce.

**Review PR2oct.3 :** `docs/reviews/review-story-PR2oct.3.md` — verdict **`VALIDÉ AVEC RÉSERVES`**, réserve unique **M1** (le schéma zod de scission acceptait un champ `alevinsAchetes` ensuite silencieusement ignoré : contrat d'API trompeur). **Réserve levée** : `.omit({ alevinsAchetes: true })` appliqué sur le schéma dérivé de scission, avec JSDoc explicite. Rapport de test : `docs/tests/rapport-story-PR2oct.3.md`. Capitalisation : **ERR-170** complétée par @knowledge-keeper.

**PR2oct.4 — `FAIT`.** Checkbox `alevinsAchetes` sur le formulaire de vague prévue (`vague-prevue-form-dialog.tsx`), préremplie depuis `alevinsAchetesParDefaut` à la création et depuis la valeur propre de la vague à l'édition ; checkbox `alevinsAchetesParDefaut` dans l'onglet Paramètres (`parametres-tab.tsx`), hors de la boucle générique `CHAMPS` dont le `Number(raw)` aurait corrompu un booléen ; badge en lecture seule « Alevins achetés » / « Production interne » sur les cartes du plan. **ADR-053 §14.4 protégé par un test dédié** (`it.each([true, false])`) : `prixAlevinUnitaireFCFA` reste **visible et éditable** quel que soit l'état du drapeau, jamais masqué ni `disabled`. i18n `fr`/`en` en parité stricte. Review : `docs/reviews/review-story-PR2oct.4.md` — verdict **`VALIDÉ AVEC RÉSERVES`**, **les deux réserves ont été levées** (vérification navigateur refaite à **360px** ; typage `tsc` du harnais de recette corrigé dans `helpers.ts`, −4 erreurs). Rapport de test : `docs/tests/rapport-story-PR2oct.4.md`.

**PR2oct.5 — `FAIT`.** Rapport : `docs/reviews/review-sprint-PR2-octies.md` — **verdict de sprint `VALIDÉ`, aucun constat Critique ni Haute**. Les trois promesses centrales ont été revérifiées par lecture directe du code : le drapeau existe et suit le §5.3 ; **la logistique alevins n'est jamais gatée** (vérifié à trois niveaux indépendants : grep du code source, test synthétique dédié, données réelles `EXCEL-V12`) ; `prixAlevinUnitaireFCFA` n'est jamais masqué ni forcé à 0.

**Vérification de fin de sprint — mesures réelles :**
- `npx prisma migrate deploy` : **168 migrations, aucune en attente**.
- `npx vitest run` : **3 passages consécutifs, compteurs identiques, 0 échec** — **289 fichiers, 8 977 tests passés** (21 skipped, 26 todo).
- `npm run build` : **OK** (`Compiled successfully`).
- Recette : **2 458 assertions / 0 écart** (base d'entrée de sprint : 2 378 — **+80**, jamais diminuée).
- Vérification en **navigateur réel** (Playwright/Chromium) à **360px** et desktop : cibles tactiles mesurées à 44px, hint de `prixAlevinUnitaireFCFA` lisible et non tronqué, badges sans débordement sur les 19 cartes, aucune erreur console (ERR-157).

**Protection des données utilisateur — snapshot APRÈS, scénario `EXCEL-V12` : aucune régression.** `VaguePrevue` 19 lignes / SUM(effectifAlevinsPrevu) = 602 500 / 0 vague à `alevinsAchetes = true` — **inchangé** ; `ApportCapital` 3 lignes — **inchangé** ; `AlimentPrevision` 3 calibres, mêmes identifiants, articles et répartitions — **inchangé** ; `ScenarioPrevision.updatedAt` `2026-08-03 20:10:26.493` — **inchangé** ; `ParametresPrevision` colonne par colonne — **seules différences : `prixAlevinUnitaireFCFA` 0 → 70 (restauration autorisée et documentée) et ajout de `alevinsAchetesParDefaut = false`**. Rapport : `docs/tests/rapport-sprint-PR2-octies.md`.

**Capitalisation @knowledge-keeper :** **ERR-170** — « Un champ des exigences absent du schéma qu'aucune recette ne peut détecter » — créée puis complétée deux fois (story PR2oct.3, puis clôture de sprint). Elle acte notamment la **preuve de discrimination par régression contrôlée** comme bonne pratique à généraliser : une assertion qui existe et passe ne prouve rien tant qu'on n'a pas montré qu'elle **casse** sur le bug qu'elle prétend attraper.

**Points ouverts consignés — à porter en backlog (PAS des stories de ce sprint, aucun bloquant) :**

| Sévérité | Point |
|---|---|
| Moyenne | La **remise alevins** du §5.3 n'est pas modélisée — choix assumé de ne pas l'inventer : aucune donnée de recette ne permettrait de discriminer une formule candidate. Un futur besoin exigera **d'abord** une donnée de recette qui l'exerce. |
| Moyenne | **1 423 erreurs `tsc --noEmit` préexistantes** dans le dépôt (1 427 avant le sprint, −4 corrigées), massivement hors module Prévisions. Dette antérieure, à trianguler séparément. |
| Moyenne | Vérifier si d'autres termes du helper de recette `src/lib/previsions/__tests__/recette/orchestration.ts` souffrent du même angle mort que `coutAlevinsFCFA` (lecture de la fixture au lieu d'un recalcul). Doute méthodique, pas un bug identifié. |
| Basse-Moyenne | 4 erreurs `tsc` résiduelles à lien indirect avec Prévisions (`RequestInit` Next vs DOM, `Decimal` vs `number`). |
| Basse | `alevinsAchetesParDefaut` absent du dialogue de **création** d'un scénario : le défaut ne peut être fixé qu'après coup, via l'onglet Paramètres. |
| Basse | Précédent de **dépassement de périmètre** (badge de `plan-vagues-tab.tsx` livré alors que la pré-analyse tranchait de ne pas l'exiger) — rappel de discipline : confirmer **avant** implémentation, pas après coup. |
