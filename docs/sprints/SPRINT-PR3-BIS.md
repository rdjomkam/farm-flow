...
| Nature | Point |
|---|---|
| ~~Risque structurel non corrigé, **hors périmètre**~~ → **SOLDÉ le 2026-08-05** | `MappingRapprochement.cibleId` était **site-scopé** alors que `PostePrevision`/`AlimentPrevision` sont **scénario-scopés** (ADR-053 §3.9), avec une simple **mitigation UI**. Le correctif structurel a été livré en deux temps : **`ALIMENT_PREVISION`** par **PR3ter.A2** (clé métier `tailleGranule`, résolution dynamique) et **`POSTE_PREVISION`** par la **story A.4** (sprint PR3-quater, modèle `PosteReferentiel`, migration `20260805120000_add_poste_referentiel`, résolution dynamique du filet **et** du moteur). Réf : ADR-053 §16, `docs/TASKS.md` §Sprint PR3-quater. |
